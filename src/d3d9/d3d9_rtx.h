...
namespace dxvk {
  ...
  struct D3D9Rtx {
    ...
    RTX_OPTION("rtx", bool, useVertexCapturedTangents, false, "When enabled, vertex tangents and binormals are captured from the vertex shader for proper normal mapping in ray tracing. Requires shader support.");
    RTX_OPTION("rtx", bool, useVertexCapturedTexcoords, false, "When enabled, captures multiple texture coordinate sets (UV0, UV1) from vertex shaders. Essential for lightmaps and detail textures.");
    RTX_OPTION("rtx", bool, useVertexCapturedColor1, false, "When enabled, captures secondary vertex color (specular color) from vertex shaders when available.");
    RTX_OPTION("rtx", fast_unordered_set, vertexShadersToCapture, {}, "Set of vertex shader hashes to target for capture (empty = all shaders). Use the shader selector in ImGui to set this.");
